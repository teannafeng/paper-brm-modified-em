"""
This script runs Simulation 1 reported in the manuscript:
...
I         = [18, 36, 54]                     # Number of items
TOL       = 1e-6                             # Convergence tolerance
DEBUG     = True                             # Has to be true for this script to work
PRM_SEED  = 2025                             # Seed for generating true item parameters
...
def run_simulation(start_iter, num_items, num_persons, num_reps, save_folder):
    ...
    for subset_perc, delta_thr, step, wait, window_chk, delta_useN in subset_conditions:
        ...
        for r in range(start_iter, num_reps + 1):
            ...
            sim_2pl.simulate(prm_seed=PRM_SEED, theta_seed=r, item_seed=r)
            ...
            sim_2pl.save(sim_save_folder, fixed_prm=True)
            ...
            # Format output data (each replication has one row)
            ...
            out = pd.concat([out, a_bias, c_bias, a_est, c_est, a_tru, c_tru, a_raw, c_raw], axis=1)
            ...
            # Save history data for the 1st replication and every 10 replications
            if r == 1 or r % 10 == 0:
        ...
        for true_col in true_columns:
            ...
            if est_col in df.columns:
                bias = np.mean(df[est_col] - df[true_col])
                rmse = np.sqrt(np.mean((df[est_col] - df[true_col]) ** 2)) 
...
if __name__ == "__main__":
    today = date.today()
    save_folder = f"./Simulation_1/{str(today)}/"
